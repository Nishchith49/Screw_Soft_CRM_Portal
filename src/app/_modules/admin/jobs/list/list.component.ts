...
import { DropDownModel, DropDownService, GetAllJobsQuery, JobModel, JobModelListPagedResponse, JobsService } from '../../../../../../dotnet/openapi';
...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._route.queryParams.subscribe(params => {
      const clientCompanyIdFromParam = params['clientCompanyId'];
      if (clientCompanyIdFromParam) {
        this.filterForm.patchValue({
          clientCompanyId: clientCompanyIdFromParam
        });
      }
  ...
  getJobsData(): void {
    const getAllJobsQuery: GetAllJobsQuery = {
    ...
    this.errorData = null;
    this.isLoadingData = true;
    this._jobsService.apiAdminJobsGetJobsPost(getAllJobsQuery).subscribe({
      next: (res: JobModelListPagedResponse) => {
  ...
  editJobDetails(jobId: any): void {
    this._router.navigate(['/admin/jobs/edit'], {
      queryParams: { jobId }
    });
  }

  enableOrDisableJob(job: JobModel): void {
    this._jobsService.apiAdminJobsEnableOrDisableJobPut({
      id: job.id
    }).subscribe();
    this._cdr.detectChanges();
  }
